...
def main():
    audio = sys.argv[1]
    example = sys.argv[2]
    reverb = "../dataset/test_latest/%s_label_synthesized_image.wav" % example
    s = audio.split("/")
    ...
    subprocess.run(["ffmpeg", "-loop", "1", "-i", "../dataset/test_latest/inputs/%s_label_input_label.jpg" % example, "-i", output + ".wav", "-c:v", "libx264", "-tune", "stillimage", "-c:a", "aac", "-b:a", "192k", "-pix_fmt", "yuv420p", "-shortest", output + ".mp4"])
